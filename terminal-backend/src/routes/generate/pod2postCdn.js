...
router.post('/', 
  ...
  try {
    ...
    const uploadedFiles = req.files.map(file => {
      ...
      let url
      if (taskId && taskId.startsWith('pod2post_')) {
        url = `/data/users/${req.user.username}/workspace/templates/pod2post/tasks/${taskId}/CDN/${file.filename}`
      } else {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stats.isFile() && /\.(jpg|jpeg|png|gif|webp|svg|bmp)$/i.test(file)) {
        ...
        let url
        if (taskId && taskId.startsWith('pod2post_')) {
          url = `/data/users/${req.user.username}/workspace/templates/pod2post/tasks/${taskId}/CDN/${file}`
        } else {
